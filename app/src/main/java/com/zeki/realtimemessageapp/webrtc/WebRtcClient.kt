...
import com.zeki.realtimemessageapp.service.RecordService
import com.zeki.realtimemessageapp.ui.home.mediaProjectionPermissionResultData
import com.zeki.realtimemessageapp.ui.home.sendMessage
import com.zeki.realtimemessageapp.ui.home.socket
import com.zeki.realtimemessageapp.utils.dip2px
...
var vc: CameraVideoCapturer? = null
...
class WebRtcClient(
    ...
    init {
        ...
        pcConstraints.mandatory.add(KeyValuePair("maxHeight", 4096.toString()))
        pcConstraints.mandatory.add(KeyValuePair("maxWidth", 2048.toString()))
...
* */
    ...
    fun startLocalCamera(context: Context) {
        ...
        vc = getFrontVideoCapture(context)
        vc?.initialize(surfaceTextureHelper, context, localVideoSource?.capturerObserver)

        serviceIntent = Intent(context, RecordService::class.java)

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            context.startForegroundService(serviceIntent)
        } else {
            //开始采集图像
            vc?.startCapture(dip2px(context,context.resources.displayMetrics.widthPixels.toFloat()), dip2px(context,context.resources.displayMetrics.heightPixels.toFloat()), 60)
        }
